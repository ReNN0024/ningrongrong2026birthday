...
- `KICKER_FONT_FAMILY` / `TENDENCY_FONT_FAMILY` / `FONT_ASSETS`：只在需要调整结果卡顶部英文标签（如 `Restrained / THORN`）的系列名、副倾向字体或本地字体文件时才改；当前系列名使用本地内嵌的 Instrument Serif Italic，副倾向使用本地内嵌的 DM Serif Display Italic。
