  ...
  async function buildSVG({ placed, logos, activityTitle, shareUrl, userId = "佚名" }) {
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${CARD_WIDTH}" height="${CARD_HEIGHT}" viewBox="0 0 ${CARD_WIDTH} ${CARD_HEIGHT}">
      ...
      <g clip-path="url(#card-clip)">
        ...
        <text x="${textXFor(style, "title").toFixed(2)}" y="${scaled(style.copy.title[1] - 50)}" text-anchor="${anchor}" font-family="sans-serif" font-size="26.67" font-weight="500" fill="${style.desc}" letter-spacing="1">${escapeXML(userIdText)}</text>
